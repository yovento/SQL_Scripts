...
- Moving data from source to destination database with deletion.
...
- Transaction control to preven data deletion on errors
...
Execution example:
EXEC Archiving_Process_With_Deletion @BatchSize = 2000, @RetentionInMonths = 30, @MinutesToRun = 2

*/

CREATE PROCEDURE Archiving_Process_With_Deletion
	@BatchSize INT,
					...
					BEGIN TRY
						BEGIN TRANSACTION						
							...
							(					
								...
								FROM tblSourceDependentTable 
								WHERE ID IN (SELECT ProcessId FROM #tmpTableToMove)			
							)
							DELETE b OUTPUT DELETED.* INTO DESTINATION_ARCHIVE_DATABASE.dbo.tblDestinationDependentTable (Field1, Field2, Field3) FROM batch b
							...
							DELETE b OUTPUT DELETED.* INTO DESTINATION_ARCHIVE_DATABASE.dbo.DestinationTable (Field1, Field2, Field3) FROM batch b

							SET @RowsCopiedToProcessTable = @TotalBatchSize;

							SET @MSG = (CONVERT( VARCHAR(24), GETDATE(), 121)) + ': Finish Moving WO, Total WO moved: ' + CAST(@TotalBatchSize AS VARCHAR)
							RAISERROR (@MSG, 0, 1) WITH NOWAIT
